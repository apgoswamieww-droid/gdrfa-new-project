...
const AgeGroupModal = ({ isOpen, onClose, onSubmit, initialData, title }: AgeGroupModalProps) => {
  ...
  const validate = () => {
    ...
    if (!formData.age_from) newErrors.age_from = t.fitness.ageFromRequired;
    if (!formData.age_to) newErrors.age_to = t.fitness.ageToRequired;
    if (Number(formData.age_from) < 0) newErrors.age_from = t.fitness.ageNotNegative;
    if (Number(formData.age_to) <= Number(formData.age_from)) newErrors.age_to = t.fitness.ageToGreaterThanFrom;
    setErrors(newErrors);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 text-start">
      <div className="bg-white rounded-xl w-full max-w-md overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          <InputField
            label={t.fitness.groupNameOptional}
            placeholder={t.fitness.placeholderName}
          ...
          <div className="grid grid-cols-2 gap-3">
            <InputField
              label={t.fitness.ageFrom}
              type="number"
              placeholder={t.fitness.placeholderAgeFrom}
            ...
            />
            <InputField
              label={t.fitness.ageTo}
              type="number"
              placeholder={t.fitness.placeholderAgeTo}
          ...
          <div className="pt-4 flex gap-3">
            ...
            <PrimaryBtn type="submit" className="flex-1" disabled={submitting}>
              {submitting ? t.fitness.saving : (initialData ? t.fitness.save : t.fitness.create)}
            </PrimaryBtn>
          </div>
        </form>
      </div>
