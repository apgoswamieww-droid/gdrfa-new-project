...
function formatTime(dateStr: string, t: any): string {
  const now = Date.now();
...
const Notifications = () => {
  ...
  const markAsRead = async (id: number) => {
    try {
      ...
      toast.error(t.notifications.errorMarkRead);
    }
  };
  ...
  const clearAll = async () => {
    ...
    try {
      ...
      setNotifications([]);
      setTotalPages(1);
      toast.success(t.notifications.successClear);
    } catch {
      toast.error(t.notifications.errorClear);
    } finally {
      setClearing(false);
    }
  };
  ...
  return (
    <div className="2xl:space-y-8 md:space-y-6 space-y-4 h-full flex flex-col">
      <div className="flex sm:flex-row flex-col gap-3 justify-between md:mb-7 mb-5">
        <h1 className="text-xl font-bold text-secondary">{t.notifications.manage}</h1>
        <div className="flex gap-2">
          ...
          >
            {t.notifications.all} {tab === "all" && `(${notifications.length})`}
          </button>
          <button
            ...
            {notifications.map(n => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-gray-400 mt-1.5">{formatTime(n.createdAt, t)}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-2 mt-6">
            ...
            <span className="text-sm text-secondary/60">
              {t.notifications.page} {page} {t.notifications.of} {totalPages}
            </span>
            <button
