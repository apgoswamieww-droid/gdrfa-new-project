...
import { getSocialLinksApi } from "../../api/socialLinks.api";
...
const StatusBadge = ({ status, onClick }: { status: string; onClick?: () => void }) => {
  ...
  return (
    ...
    >
      ...
      {isActive ? t.socialLinks.active : t.socialLinks.inactive}
    </button>
  );
};
...
interface SocialLinksTableProps {
  searchTerm?: string;
  onEdit: (socialLink: any) => void;
  onDelete: (id: number) => void;
}

export default function SocialLinksTable({ searchTerm, onEdit, onDelete }: SocialLinksTableProps) {
  const { t } = useTranslation();
  const [data, setData] = useState<any[]>([]);

  const fetchSocialLinks = async () => {
    try {
      const response = await getSocialLinksApi({ search: searchTerm });
      if (response.status) {
  ...
  const handleToggleStatus = async (row: any) => {
    ...
    const loadingToast = toast.loading(t.socialLinks.updatingStatus);
    try {
      const res = await changeStatusApi("SocialLink", row.id, newStatus);
      if (res.status) {
        toast.success(t.socialLinks.statusUpdated, { id: loadingToast });
        fetchSocialLinks();
      } else {
        toast.error(res.message || t.socialLinks.failedStatus, { id: loadingToast });
      }
    } catch (error: any) {
      toast.error(error.message || t.socialLinks.failedStatus, { id: loadingToast });
    }
  };
  ...
  const columns: Column<any>[] = [
    ...
    {
      key: "image",
      label: t.socialLinks.icon,
      sortable: false,
      className: "text-center",
      render: (_, r) => (
        <div className="flex items-center justify-center">
          {r.image_url ? (
            <img src={r.image_url} alt="icon" className="w-10 h-10 rounded-lg object-cover border shrink-0 bg-white" />
          ) : (
            <div className="w-10 h-10 rounded-lg bg-[#eef0f8] flex items-center justify-center text-xs font-bold text-secondary shrink-0">
              -
            </div>
          )}
        </div>
    ...
    },
    {
      ...
      render: (value) => (
        <a href={value as string} target="_blank" rel="noopener noreferrer" className="text-light-blue underline hover:no-underline">
          {value || "-"}
        </a>
      ),
    },
