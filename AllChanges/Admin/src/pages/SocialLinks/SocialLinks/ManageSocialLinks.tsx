...
import { createSocialLinkApi, updateSocialLinkApi, deleteSocialLinkApi } from "../../api/socialLinks.api";
...
const ManageSocialLinks = () => {
  ...
  const [editingSocialLink, setEditingSocialLink] = useState<any | undefined>(undefined);
  const [refreshKey, setRefreshKey] = useState(0);

  const handleCreateOrUpdateSocialLink = async (formData: FormData) => {
    const loadingToast = toast.loading(editingSocialLink ? t.socialLinks.updating : t.socialLinks.creating);
    try {
      if (editingSocialLink) {
        await updateSocialLinkApi(editingSocialLink.id, formData);
        toast.success(t.socialLinks.successUpdate, { id: loadingToast });
      } else {
        await createSocialLinkApi(formData);
        toast.success(t.socialLinks.successCreate, { id: loadingToast });
      }
  ...
  const handleEditSocialLink = (socialLink: any) => {
  ...
  const handleDeleteSocialLink = async (id: number) => {
    toast((t_toast) => (
      <div className="flex flex-col gap-3 p-1">
        <p className="font-bold text-secondary text-base text-start">{t.socialLinks.confirmDelete}</p>
        <div className="flex gap-2 justify-end">
          ...
          <button
            onClick={async () => {
              ...
              const loadingToast = toast.loading(t.socialLinks.deleting);
              try {
                await deleteSocialLinkApi(id);
                toast.success(t.socialLinks.successDelete, { id: loadingToast });
                setRefreshKey((prev) => prev + 1);
              } catch (error: any) {
                console.error("Failed to delete social link:", error);
                toast.error(error.message || "Failed to delete social link", { id: loadingToast });
              }
            }}
    ...
    <>
      <div className="h-full flex flex-col">
        <div className="flex sm:flex-row flex-col gap-3 justify-between mb-5">
          <SearchInput
            placeholder={t.socialLinks.search}
            value={searchTerm}
      ...
      <SocialLinksModal
        ...
        onSubmit={handleCreateOrUpdateSocialLink}
        initialData={editingSocialLink}
        title={editingSocialLink ? t.socialLinks.edit : t.socialLinks.create}
      />
    </>
  );
};
