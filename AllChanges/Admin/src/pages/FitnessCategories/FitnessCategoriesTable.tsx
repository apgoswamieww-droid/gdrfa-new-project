...
import { getFitnessCategoriesApi } from "../../api/fitness.api";
...
export default function FitnessCategoriesTable({
  searchTerm,
  onEdit,
  onDelete,
  onToggleStatus
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const res = await getFitnessCategoriesApi({ search: searchTerm, length: 100 });
        if (res.status) {
          setData(res.data.data);
        }
      } catch (e) {
        console.warn("Fitness Categories fetch error:", e);
      } finally {
        setLoading(false);
      }
    };
  ...
  const columns: Column<any>[] = useMemo(() => [
    ...
    {
      key: "slug",
      label: t.fitness.slug,
      sortable: true,
      className: "text-[#898B8E] 2xl:text-base/tight text-base/tight font-medium",
      render: (value) => (
        <span className="inline-flex px-2 py-1 bg-gray-100 rounded text-xs font-mono">{String(value ?? "")}</span>
      ),
    },
    {
      key: "unit_type",
      label: t.fitness.unitType,
      sortable: true,
      className: "text-center",
      render: (value) => (
        <span className={`px-2 py-1 rounded text-xs font-bold ${value === "time" ? "bg-blue-50 text-blue-600" : "bg-purple-50 text-purple-600"}`}>
          {value === "time" ? t.fitness.time : t.fitness.count}
        </span>
      ),
    },
    {
      key: "status",
      label: t.fitness.status,
      sortable: true,
      className: "text-center",
      render: (value, row) => {
        ...
        return (
          <button
            onClick={() => onToggleStatus(row.id, value as string)}
          ...
          >
            ...
            {isActive ? t.fitness.active : t.fitness.inactive}
          </button>
        );
      },
    },
