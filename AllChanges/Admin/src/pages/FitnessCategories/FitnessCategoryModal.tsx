...
const FitnessCategoryModal = ({ isOpen, onClose, onSubmit, initialData, title }: FitnessCategoryModalProps) => {
  ...
  const validate = () => {
    ...
    if (!formData.name.trim()) newErrors.name = t.fitness.nameRequired;
    if (!formData.slug.trim()) newErrors.slug = t.fitness.slugRequired;
    if (!formData.unit_type) newErrors.unit_type = t.fitness.unitTypeRequired;
    setErrors(newErrors);
      ...
      <div className="bg-white rounded-xl w-full max-w-md overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          ...
          <InputField
            ...
            placeholder={t.fitness.placeholderSlug}
            value={formData.slug}
          ...
          <div>
            <label className="block text-sm font-semibold text-secondary/70 mb-1.5">{t.fitness.unitType}</label>
            <div className="flex gap-4">
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                />
                <span className="text-sm font-medium text-gray-700">{t.fitness.count}</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="radio"
                  ...
                  className="w-4 h-4 text-primary"
                />
                <span className="text-sm font-medium text-gray-700">{t.fitness.time}</span>
              </label>
            </div>
