    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 text-start">
      <div className="bg-white rounded-xl w-full max-w-2xl overflow-hidden">
        <div className="p-4 border-b border-gray-100 flex justify-between items-center">
          <div>
            <h2 className="text-lg font-bold text-secondary">{t.scoreMatrix.bulk.title}</h2>
            <p className="text-xs text-gray-400 mt-0.5">{t.scoreMatrix.bulk.subtitle}</p>
          </div>
        ...
        <div className="p-4 space-y-4">
          <div>
            <label className="block text-sm font-semibold text-secondary/70 mb-1.5">{t.scoreMatrix.bulk.uploadLabel}</label>
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => fileInputRef.current?.click()} className="text-xs text-primary font-semibold hover:underline">{t.scoreMatrix.bulk.chooseFile}</button>
            </div>
          </div>

          <div>
            <label className="block text-sm font-semibold text-secondary/70 mb-1.5">{t.scoreMatrix.bulk.orPaste}</label>
            <textarea
              value={entries}
          ...
          <div className="bg-gray-50 rounded-lg p-3 border border-gray-100">
            <p className="text-xs font-semibold text-gray-500 mb-1">{t.scoreMatrix.bulk.expectedColumns}</p>
            <code className="text-xs text-primary">category_id, gender, age_group_id, score, min_value, max_value</code>
            <p className="text-xs text-gray-400 mt-1">{t.scoreMatrix.bulk.alternative}</p>
            <p className="text-xs text-gray-400">{t.scoreMatrix.bulk.timeHint}</p>
          </div>

          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} className="flex-1 flex justify-center items-center font-bold text-sm rounded-lg px-6 border border-gray-200 py-2 text-gray-600 hover:bg-gray-50 transition-all cursor-pointer">{t.scoreMatrix.bulk.cancel}</button>
            <PrimaryBtn className="flex-1" onClick={handleSubmit} disabled={submitting || !entries.trim()}>
              {submitting ? t.scoreMatrix.bulk.importing : `${entries.trim() ? (detectAndParse(entries).length || '...') : 0} ${t.scoreMatrix.bulk.importEntries}`}
            </PrimaryBtn>
          </div>
        </div>
      </div>
