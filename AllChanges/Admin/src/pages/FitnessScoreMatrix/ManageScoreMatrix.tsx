...
const ManageScoreMatrix = () => {
  ...
  const columns: Column<any>[] = useMemo(() => [
    { key: "id", label: t.scoreMatrix.id, sortable: true, className: "text-center w-12 text-[#898B8E] font-medium", render: (value) => String(value ?? "") },
    {
      key: "category_id",
      label: t.scoreMatrix.category,
      sortable: true,
    ...
    {
      key: "gender",
      label: t.scoreMatrix.gender,
      sortable: true,
      className: "text-center",
      render: (value) => (
        <span className={`px-2 py-1 rounded text-xs font-bold ${value === "male" ? "bg-blue-50 text-blue-600" : "bg-pink-50 text-pink-600"}`}>
          {value === "male" ? t.scoreMatrix.male : t.scoreMatrix.female}
        </span>
      ),
    },
    {
      key: "age_group_id",
      label: t.scoreMatrix.ageGroup,
      sortable: true,
      render: (value) => ageMap[value as number] || `#${value}`,
    },
    { key: "score", label: t.scoreMatrix.score, sortable: true, className: "text-center font-bold text-primary", render: (value) => String(value ?? "") },
    {
      key: "min_value",
      label: t.scoreMatrix.min,
      sortable: true,
      className: "text-center",
      render: (value, row) => {
    ...
    { key: "createdAt", label: t.scoreMatrix.created, sortable: true, className: "text-[#898B8E] whitespace-nowrap", render: (value) => formatDate(value as string) },
  ], [catMap, ageMap, t]);

  const handleCreateOrUpdate = async (data: any) => {
    const loadingToast = toast.loading(editingItem ? t.scoreMatrix.updating : t.scoreMatrix.creating);
    try {
      if (editingItem) {
        await updateScoreMatrixApi(editingItem.id, data);
        toast.success(t.scoreMatrix.successUpdate, { id: loadingToast });
      } else {
        await createScoreMatrixApi(data);
        toast.success(t.scoreMatrix.successCreate, { id: loadingToast });
      }
      setIsModalOpen(false);
      setEditingItem(undefined);
      setRefreshKey((prev) => prev + 1);
    } catch (error: any) {
      toast.error(error.message || t.scoreMatrix.errorSomethingWrong, { id: loadingToast });
    }
  };
  ...
  const handleDelete = async (id: number) => {
    toast((t_toast) => (
      <div className="flex flex-col gap-3 p-1 text-start">
        <p className="font-bold text-secondary text-base">{t.scoreMatrix.confirmDelete}</p>
        <div className="flex gap-2 justify-end">
          <button onClick={() => toast.dismiss(t_toast.id)} className="px-4 py-2 rounded-lg text-sm font-bold text-gray-600 border border-gray-200 hover:bg-gray-50 transition-colors cursor-pointer">{t.scoreMatrix.cancel}</button>
          <button onClick={async () => {
            toast.dismiss(t_toast.id);
            const loadingToast = toast.loading(t.scoreMatrix.deleting);
            try {
              await deleteScoreMatrixApi(id);
              toast.success(t.scoreMatrix.successDelete, { id: loadingToast });
              setRefreshKey((prev) => prev + 1);
            } catch (error: any) {
              toast.error(error.message || t.scoreMatrix.errorDelete, { id: loadingToast });
            }
          }} className="px-4 py-2 rounded-lg text-sm font-bold bg-primary text-white hover:bg-primary/90 transition-colors cursor-pointer">{t.scoreMatrix.delete}</button>
        </div>
      </div>
  ...
  const actions = (row: any) => (
    <div className="flex items-center gap-1.5">
      <button title={t.fitness.edit} onClick={() => { setEditingItem(row); setIsModalOpen(true); }} className="min-w-8 w-8 h-8 flex items-center justify-center rounded-lg bg-light-blue/20 transition-colors cursor-pointer hover:bg-light-blue/10"><EditIcon /></button>
      <span className="text-primary text-base">|</span>
      <button title={t.fitness.delete} onClick={() => handleDelete(row.id)} className="min-w-8 w-8 h-8 flex items-center justify-center rounded-lg bg-primary/20 transition-colors cursor-pointer hover:bg-primary/10"><DeleteIcon /></button>
    </div>
  );
  ...
  return (
    <>
      <div className="space-y-6 h-full flex flex-col">
        <div className="flex flex-col gap-3">
          <div className="flex sm:flex-row flex-col gap-3 justify-between">
            <SearchInput placeholder={t.scoreMatrix.search} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
            <div className="flex gap-2">
              <PrimaryBtn className="w-fit !bg-white !text-primary !border !border-primary/30" onClick={() => setIsBulkOpen(true)}>
                {t.scoreMatrix.bulkImport}
              </PrimaryBtn>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            >
              <option value="">{t.scoreMatrix.allCategories}</option>
            ...
            <select
              ...
              className="px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-primary/20"
            >
              <option value="">{t.scoreMatrix.allGenders}</option>
              <option value="male">{t.scoreMatrix.male}</option>
              <option value="female">{t.scoreMatrix.female}</option>
            </select>
            <select
              ...
              className="px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-primary/20"
            >
              <option value="">{t.scoreMatrix.allAgeGroups}</option>
      ...
      <ScoreMatrixModal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); setEditingItem(undefined); }} onSubmit={handleCreateOrUpdate} initialData={editingItem} title={editingItem ? t.scoreMatrix.editEntry : t.scoreMatrix.createEntry} />
