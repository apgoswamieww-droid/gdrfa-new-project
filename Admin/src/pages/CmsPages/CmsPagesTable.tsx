...
import { getCmsPagesApi } from "../../api/cms.api";
...
export default function CmsPagesTable({ 
  ...
  useEffect(() => {
    const fetchData = async () => {
        setLoading(true);
        try {
          const res = await getCmsPagesApi({ search: searchTerm, length: 100 });
          if (res.status) {
            setData(res.data.data);
          }
        } catch (e) {
          console.warn("CMS Pages fetch error:", e);
        } finally {
          setLoading(false);
        }
    };
  ...
  const columns: Column<CmsPage>[] = useMemo(() => [
    {
      key: "id",
      label: t.cms.id,
      sortable: true,
      ...
      render: (value, row) => {
        ...
        return (
          ...
          >
            ...
            {isActive ? t.cms.active : t.cms.inactive}
          </button>
        );
      },
    },
  ...
  const actions = (row: CmsPage) => (
    <div className="flex items-center gap-1.5">
      <button
        title={t.common.view}
        onClick={() => navigate(`/cms/pages/view/${row.id}`)}
