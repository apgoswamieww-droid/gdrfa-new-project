...
import { createTeamApi, updateTeamApi, deleteTeamApi } from "../../api/teams.api";
...
const ManageTeam = () => {
  ...
  const handleCreateOrUpdateTeam = async (formData: FormData) => {
    const loadingToast = toast.loading(editingTeam ? t.team.updating : t.team.creating);
    try {
      if (editingTeam) {
        await updateTeamApi(editingTeam.id, formData);
        toast.success(t.team.successUpdate, { id: loadingToast });
      } else {
        await createTeamApi(formData);
        toast.success(t.team.successCreate, { id: loadingToast });
      }
  ...
  const handleDeleteTeam = async (id: number) => {
    toast((t_toast) => (
      <div className="flex flex-col gap-3 p-1">
        <p className="font-bold text-secondary text-base text-start">{t.team.confirmDelete}</p>
        <div className="flex gap-2 justify-end">
          ...
          <button
            onClick={async () => {
              ...
              const loadingToast = toast.loading(t.team.deleting);
              try {
                await deleteTeamApi(id);
                toast.success(t.team.successDelete, { id: loadingToast });
                setRefreshKey((prev) => prev + 1);
              } catch (error: any) {
