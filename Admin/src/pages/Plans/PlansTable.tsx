...
import { getPlansApi, deletePlanApi, changePlanStatusApi } from "../../api/plans.api";
...
const StatusBadge = ({ status, onClick }: { status: string; onClick?: () => void }) => {
  ...
  return (
    ...
    >
      ...
      {isActive ? t.plan.active : t.plan.inactive}
    </button>
  );
};

// ─── Main Component ───────────────
export default function PlansTable({ searchTerm, onEdit, onRefresh, kpiOptions = [] }: { 
  searchTerm: string; 
  ...
  const handleToggleStatus = async (row: Plan) => {
    ...
    const loadingToast = toast.loading(t.plan.updating || "Updating...");
    try {
      const res = await changePlanStatusApi(row.id, newStatus);
      if (res.status) {
        toast.success(t.plan.successUpdate || "Status updated successfully", { id: loadingToast });
        fetchPlans();
      }
    } catch (error: any) {
      console.error("Failed to toggle status:", error);
      toast.error(error.message || t.plan.errorFetch || "Failed to update status", { id: loadingToast });
    }
  };
  ...
  const handleDeletePlan = (id: number) => {
    toast((t_toast) => (
      <div className="flex flex-col gap-3 p-1">
        <p className="font-bold text-secondary text-base text-start">{t.plan.confirmDelete || "Are you sure you want to delete this plan?"}</p>
        <div className="flex gap-2 justify-end">
          ...
          <button
            onClick={async () => {
              ...
              const loadingToast = toast.loading(t.plan.deleting || "Deleting...");
              try {
                await deletePlanApi(id);
                toast.success(t.plan.successDelete || "Plan deleted successfully", { id: loadingToast });
                fetchPlans();
                onRefresh();
              } catch (error: any) {
                console.error("Failed to delete Plan:", error);
                toast.error(error.message || t.plan.errorFetch || "Failed to delete", { id: loadingToast });
              }
            }}
  ...
  const columns: Column<Plan>[] = [
    ...
    {
      key: "createdAt",
      label: t.plan.createdAt || "Created At",
      sortable: true,
  ...
  if (loading) {
    return <div className="bg-white rounded-xl p-5 text-center text-gray-400">{t.plan.loading || "Loading..."}</div>;
  }

  if (error) {
    return <div className="bg-white rounded-xl p-5 text-center text-red-500">{t.plan.errorFetch || "Failed to load plans"}</div>;
  }
