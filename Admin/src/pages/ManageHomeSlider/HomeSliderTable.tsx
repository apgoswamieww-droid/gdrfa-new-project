...
import { getHomeSlidersApi } from "../../api/homeSlider.api";
...
const StatusBadge = ({ status, onClick }: { status: string; onClick?: () => void }) => {
  ...
  return (
    ...
    >
      ...
      {isActive ? t.homeSlider.active : t.homeSlider.inactive}
    </button>
  );
};
...
interface HomeSliderTableProps {
  searchTerm?: string;
  onEdit: (slider: any) => void;
  onDelete: (id: number) => void;
}

export default function HomeSliderTable({ searchTerm, onEdit, onDelete }: HomeSliderTableProps) {
  const { t } = useTranslation();
  const [data, setData] = useState<any[]>([]);

  const fetchSliders = async () => {
    try {
      const response = await getHomeSlidersApi({ search: searchTerm });
      if (response.status) {
        setData(response.data.data);
      }
    } catch (error: any) {
      toast.error(error.message || "Failed to fetch home sliders");
    }
  };
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((s) =>
      ...
      (s.title_ar || "").toLowerCase().includes(searchTerm.toLowerCase())
    );
  }, [data, searchTerm]);
