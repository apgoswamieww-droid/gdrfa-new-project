...
import { getSponsorsApi } from "../../api/sponsors.api";
...
const StatusBadge = ({ status, onClick }: { status: string; onClick?: () => void }) => {
  ...
  return (
    ...
    >
      ...
      {isActive ? t.sponsor.active : t.sponsor.inactive}
    </button>
  );
};
...
export default function SponsorTable({ searchTerm, onEdit, onDelete }: SponsorTableProps) {
  const { t } = useTranslation();
  const [data, setData] = useState<any[]>([]);

  const fetchSponsors = async () => {
    try {
      const response = await getSponsorsApi({ search: searchTerm });
      if (response.status) {
    ...
    },
    {
      ...
      render: (_, r) => (
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium text-black 2xl:text-base/tight text-base/tight">{r.name}</span>
        </div>
      ),
    },
    {
