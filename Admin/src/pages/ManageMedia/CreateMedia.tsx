...
const CreateMedia = () => {
  ...
  const validate = (field?: string) => {
    ...
    if (!file) newErrors.file = t.media.fileRequired;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validate()) return;
    setSubmitting(true);
    const loadingToast = toast.loading(t.media.creating);
    try {
      ...
      toast.success(t.media.successCreate, { id: loadingToast });
      navigate("/cms/media");
    } catch (error: any) {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="w-full bg-white rounded-xl p-4 shadow-sm border border-gray-100">
        <div className="mb-6">
          <h2 className="text-xl font-bold text-secondary">{t.media.create}</h2>
        </div>
            ...
            <div className="relative border-2 border-dashed border-[#364B9B66] rounded-lg p-4 flex flex-col items-center justify-center gap-2 cursor-pointer hover:bg-gray-50 transition-colors">
              ...
              {filePreview ? (
                ...
                <div className="flex flex-col items-center gap-1 text-center">
                  ...
                  <span className="text-sm font-medium text-secondary">{t.media.uploadHint}</span>
