...
const ViewMedia = () => {
  ...
  const renderPreview = () => {
    if (!media.file_url) return <p className="text-gray-400 italic">{t.media.noFile}</p>;
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="w-full bg-white rounded-xl p-4 shadow-sm border border-gray-100">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-secondary">{t.media.mediaDetails}</h2>
          <span className={`inline-flex items-center gap-1.5 px-3 py-2 rounded-full text-sm font-semibold ${media.status === "1" ? "bg-primary-green/8 text-primary-green" : "bg-red-50 text-red-600"}`}>
            ...
            {media.status === "1" ? t.media.active : t.media.inactive}
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-secondary/50 mb-1">{t.media.titleEn}</label>
              <p className="text-secondary font-medium">{media.title}</p>
            </div>
            <div>
              <label className="block text-xs font-semibold text-secondary/50 mb-1">{t.media.descPlaceholder}</label>
              <p className="text-gray-600">{media.description}</p>
            </div>
            <div>
              <label className="block text-xs font-semibold text-secondary/50 mb-1">{t.media.fileType}</label>
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-secondary/50 mb-1">{t.media.titleAr}</label>
              <p className="text-secondary font-medium" dir="rtl">{media.title_ar || "-"}</p>
            </div>
            <div>
              <label className="block text-xs font-semibold text-secondary/50 mb-1">{t.media.descArPlaceholder}</label>
        ...
        {media.tags && media.tags.length > 0 && (
          <div className="mt-6">
            <label className="block text-xs font-semibold text-secondary/50 mb-2">{t.media.tags}</label>
        ...
        <div className="mt-6">
          <label className="block text-xs font-semibold text-secondary/50 mb-2">{t.media.filePreview}</label>
          {renderPreview()}
        </div>

        <div className="mt-6 pt-4 border-t border-gray-100 grid grid-cols-2 gap-3 text-sm text-gray-500">
          <div>
            <span className="font-semibold">{t.media.created}</span> {formatDate(media.createdAt)}
          </div>
          <div>
            <span className="font-semibold">{t.media.updated}</span> {formatDate(media.updatedAt)}
          </div>
        </div>
