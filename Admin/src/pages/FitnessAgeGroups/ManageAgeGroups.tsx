...
import { getAgeGroupsApi, createAgeGroupApi, updateAgeGroupApi, deleteAgeGroupApi } from "../../api/fitness.api";
...
const ManageAgeGroups = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await getAgeGroupsApi({ search: searchTerm });
      if (res.status) {
        setData(Array.isArray(res.data) ? res.data : (res.data.data || []));
      }
    } catch (e) {
      console.warn("Age groups fetch error:", e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const columns: Column<any>[] = useMemo(() => [
    {
      ...
      label: "ID",
      sortable: true,
      className: "text-center w-12 text-[#898B8E] font-medium",
    ...
    },
    {
      ...
      label: "Group Name",
      sortable: true,
      className: "font-medium text-black min-w-40",
      render: (value) => value || "-",
    },
    {
      key: "age_from",
    ...
    },
    {
      ...
      label: "Created At",
      sortable: true,
      className: "text-[#898B8E] whitespace-nowrap",
  ...
  const handleCreateOrUpdate = async (data: any) => {
    ...
    try {
      if (editingItem) {
        await updateAgeGroupApi(editingItem.id, data);
        toast.success("Age group updated successfully!", { id: loadingToast });
      } else {
        await createAgeGroupApi(data);
        toast.success("Age group created successfully!", { id: loadingToast });
      }
  ...
  const handleDelete = async (id: number) => {
    toast((t_toast) => (
      <div className="flex flex-col gap-3 p-1 text-start">
        <p className="font-bold text-secondary text-base">Are you sure you want to delete this age group?</p>
        <div className="flex gap-2 justify-end">
          ...
          <button onClick={async () => {
            ...
            try {
              await deleteAgeGroupApi(id);
              toast.success("Age group deleted successfully!", { id: loadingToast });
              setRefreshKey((prev) => prev + 1);
            } catch (error: any) {
    ...
    <>
      <div className="space-y-6 h-full flex flex-col">
        <div className="flex sm:flex-row flex-col gap-3 justify-between mb-5">
          <SearchInput placeholder="Search age groups..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        ...
        <DataTable data={data} columns={columns} actions={actions} perPageOptions={[5, 10, 20]} className="flex-1" loading={loading} />
      </div>
      ...
      <AgeGroupModal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); setEditingItem(undefined); }} onSubmit={handleCreateOrUpdate} initialData={editingItem} title={editingItem ? "Edit Age Group" : "Create Age Group"} />
    </>
  );
};
