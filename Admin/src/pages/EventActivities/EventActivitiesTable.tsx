...
import type { EventActivity } from "../../api/event-activities.api";
import { getEventActivitiesApi, deleteEventActivityApi, changeEventActivityStatusApi } from "../../api/event-activities.api";
...
const StatusBadge = ({ status, onClick }: { status: string; onClick?: () => void }) => {
  ...
  return (
    ...
    >
      ...
      {isActive ? t.eventActivity?.active || "Active" : t.eventActivity?.inactive || "Inactive"}
    </button>
  );
};

// ─── Main Component ────────────────────────────────────────────────
export default function EventActivitiesTable({ searchTerm, onEdit, onRefresh }: { searchTerm: string; onEdit: (data: EventActivity) => void; onRefresh: () => void }) {
  const { t } = useTranslation();
  const [data, setData] = useState<EventActivity[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(false);

  // Fetch Event Activities function
  const fetchEventActivities = () => {
    setLoading(true);
    getEventActivitiesApi()
      .then((res) => {
        if (res?.data?.data) setData(res.data.data);
      })
      .catch((e) => {
        console.warn("Event Activity fetch error:", e);
        setError(true);
      })
  ...
  const handleToggleStatus = async (row: EventActivity) => {
    const newStatus = row.status === "1" ? "0" : "1";
    const loadingToast = toast.loading(t.eventActivity?.updating || "Updating...");
    try {
      const res = await changeEventActivityStatusApi(row.id, newStatus);
      if (res.status) {
        toast.success(t.eventActivity?.successUpdate || "Updated successfully", { id: loadingToast });
        fetchEventActivities();
      }
    } catch (error: any) {
      console.error("Failed to toggle status:", error);
      toast.error(error.message || t.eventActivity?.errorFetch || "Error occurred", { id: loadingToast });
    }
  };

  const handleDeleteEventActivity = (id: number) => {
    toast((t_toast) => (
      <div className="flex flex-col gap-3 p-1">
        <p className="font-bold text-secondary text-base text-start">{t.eventActivity?.confirmDelete || "Are you sure you want to delete this Activity?"}</p>
        <div className="flex gap-2 justify-end">
          ...
          >
            {t.eventActivity?.cancel || "Cancel"}
          </button>
          <button
            onClick={async () => {
              toast.dismiss(t_toast.id);
              const loadingToast = toast.loading(t.eventActivity?.deleting || "Deleting...");
              try {
                await deleteEventActivityApi(id);
                toast.success(t.eventActivity?.successDelete || "Deleted successfully", { id: loadingToast });
                fetchEventActivities();
                onRefresh();
              } catch (error: any) {
                console.error("Failed to delete Event Activity:", error);
                toast.error(error.message || t.eventActivity?.errorFetch || "Error occurred", { id: loadingToast });
              }
            }}
  ...
  const columns: Column<EventActivity>[] = [
    {
      key: "id",
      label: t.eventActivity?.id || "ID",
      sortable: true,
    ...
    {
      key: "name",
      label: t.eventActivity?.name || "Activity Name",
      sortable: true,
    ...
    {
      key: "activityType",
      label: t.eventActivity?.activityType || "Event Type",
      sortable: true,
      className: "text-[#898B8E] 2xl:text-base/tight text-base/tight",
      render: (_, row) => row.activityTypeName || "N/A",
    },
    {
      key: "isTeam",
      label: t.eventActivity?.isTeam || "Is Team",
      sortable: true,
    ...
    {
      key: "status",
      label: t.eventActivity?.status || "Status",
      sortable: true,
      className: "text-center",
    ...
    {
      key: "createdAt",
      label: t.eventActivity?.createdAt || "Created At",
      sortable: true,
  ...
  const actions = (row: EventActivity) => (
    <div className="flex items-center gap-1.5">
      <button
        title={t.eventActivity?.edit || "Edit"}
        onClick={() => onEdit(row)}
      ...
      <button
        title={t.eventActivity?.delete || "Delete"}
        onClick={() => handleDeleteEventActivity(row.id)}
  ...
  if (loading) {
    return <div className="bg-white rounded-xl p-5 text-center text-gray-400">{t.eventActivity?.loading || "Loading..."}</div>;
  }

  if (error) {
    return <div className="bg-white rounded-xl p-5 text-center text-red-500">{t.eventActivity?.errorFetch || "Error loading data"}</div>;
  }
